...
public class ThirdExercise {
    ...
    void openMailHomePage() {
        ...
        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@data-test-id = 'send']")))
            .click();
        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@class= 'ico ico_16-close ico_size_s']"))).click();

        //проверить что письмо в входящие
        WebElement incoming = driver.findElement(By.xpath("//*[@href = '/tomyself/?']"));
        incoming.click(); // открыть папку входящие
        WebElement correspondentInc = driver.findElement(By.xpath("//*[@class = 'llct__correspondent llct__correspondent_new']"));
        WebElement subjectInc = driver.findElement(By.xpath("//*[@class = 'll-sj__normal']"));
        WebElement bodyInc = driver.findElement(By.xpath("//*[@class = 'llct__snippet']"));

        // Assertions.assertThat(incoming.getText()).isEqualTo("Письма себе"); ?
        //Assertions.assertThat(correspondentInc.getText()).isEqualTo("Test Testovich");??
        Assertions.assertThat(subjectInc.getText()).isEqualTo("Test");
        Assertions.assertThat(bodyInc.getText()).isEqualTo("test body -- Test Testovich Отправлено из Почты Mail.ru");

        subjectInc.click();
        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@data-title-shortcut= 'Del']"))).click();

        WebElement trash = driver.findElement(By.xpath("//*[@href = '/trash/?']"));
        trash.click(); // открыть папку отправленные
        ...
        //Assertions.assertThat(trash.getText()).isEqualTo("Корзина Очистить");?
        Assertions.assertThat(correspondent1.getText()).isEqualTo("Test Testovich");
        Assertions.assertThat(subject1.getText()).isEqualTo("Test");
